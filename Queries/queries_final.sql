...
CREATE TABLE repair (
    ...
    item_repair_code INT,
    item_warranty INT,
    warranty_usage TEXT,
    PRIMARY KEY (item_repair_code)
);
...
FROM
    electronic_store_management.user
        INNER JOIN
    electronic_store_management.repair ON user.user_id = repair.user_id;
...
FROM
    electronic_store_management.item
        INNER JOIN
    electronic_store_management.purchase ON item.item_id = purchase.item_id;
...
FROM
    electronic_store_management.item
        LEFT JOIN
    electronic_store_management.repair ON item.item_id = repair.item_id;
...
SELECT 
    ...
    item.item_version,
    item.item_warranty
FROM ((electronic_store_management.user INNER JOIN electronic_store_management.purchase ON user.user_name = purchase.user_name) INNER JOIN electronic_store_management.item ON purchase.item_id=item.item_id);
...
FROM
    electronic_store_management.purchase;
...
SELECT user_id,item_id,item_type,purchase_id,date_of_purchase FROM electronic_store_management.repair
INTERSECT
SELECT user_id,item_id,item_type,purchase_id,date_of_purchase FROM electronic_store_management.purchase;
...
SELECT user_id FROM electronic_store_management.user UNION ALL SELECT user_id FROM electronic_store_management.repair; 


-- 4. MINUS OF USER AND PURCHASE TABLEs
SELECT user_id,user_name FROM electronic_store_management.user EXCEPT SELECT user_id,user_name FROM electronic_store_management.purchase;
...
-- create table person_backup ( user_name TEXT, user_age INT, user_id INT, user_gender TEXT, user_phone TEXT);  
-- DELETE FROM electronic_store_management.user WHERE user_id=2;
...
-- DELETE FROM electronic_store_management.item WHERE item_id=8;
...
from ((electronic_store_management.user INNER JOIN repair ON user.user_id=repair.user_id)
...
-- 1. To change the purchase amount based on the discount
...
-- 2. To determine what percentage of discount a user gets based on their previous purchases
